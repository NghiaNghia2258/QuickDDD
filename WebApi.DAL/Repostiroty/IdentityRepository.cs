...
namespace WebApi.DAL.Repostiroty
{
    public class IdentityRepository : RepositoryBase<UserLogin, int>, IIdentityRepository
    {
        ...
        public async Task<List<UserLogin>> GetAllUsersAsync(OptionFilterUser option)
        {
            ...
            var query = _dbContext.UserLogins.AsQueryable();

            return await query.Skip(option.PageSize * (option.PageIndex - 1))
        ...
        {
            return await _dbContext.UserLogins.FirstOrDefaultAsync(u => u.Username == username);
        }
        public async Task<bool> UpdateAccount(UserLogin user)
        {
